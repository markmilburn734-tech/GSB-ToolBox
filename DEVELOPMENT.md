...
- **Fonts (IBM Plex superfamily, loaded via Google Fonts in `index.html`):** `font-serif` = IBM Plex Serif (page titles/headings), `font-sans` = IBM Plex Sans (UI/body, the default), `font-mono` = IBM Plex Mono (figures/codes/ISINs). Purpose-matched: serif for display, sans for reading, mono for numbers.
...
- **Header:** dark-purple (`#2e1c34`) sticky nav with the brand-gradient accent line along its bottom edge; the sub-tab bar below is **white**. Wordmark white, GSB mark in pink (`brand3`, no tile). Currency is a **custom branded dropdown** — transparent trigger with a **pink (`brand3`) border** + white text; menu items highlight pink on active/hover (not a native `<select>`).
...
- **Tables outlined + translucent:** all data-table cards (Standard/PB rebalancers, Portfolios, IHT estate/gifts/life, CGT) use a **dark-purple outline** (`border-brand`). Main panels use **translucent glass** (`bg-white/75 backdrop-blur-sm`) so the centred watermark shows through. Print paths carry `print:bg-white`. (Done on the tables + the two rebalancers' panels; other side-panels can be given the same treatment.)
