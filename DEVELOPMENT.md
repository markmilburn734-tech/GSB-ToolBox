...
- **Fonts:** `font-serif` = IBM Plex Serif (page titles), `font-sans` = IBM Plex Sans (UI/body, default) — both loaded via Google Fonts in `index.html`. `font-mono` = **Cascadia Code → Consolas** (native, figures/codes/ISINs) — IBM Plex Mono was tried for numbers and rejected as ugly.
