...
- **Tabs (text-only, no icons):** primary tabs (`TabButton`) = white text + **pink** (`brand3`) active underline on the dark banner; **sub-tabs active = dark-purple bg + pink text** (`bg-[#2e1c34] text-brand3`) on the white sub-bar, inactive grey.
- **Main tables:** the primary data-table cards (Standard/PB rebalancers, Portfolios, IHT estate, CGT disposals) use a **dark-purple outline** (`border-brand`) instead of grey.
