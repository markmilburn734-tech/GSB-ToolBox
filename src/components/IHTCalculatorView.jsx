...
export default function IHTCalculatorView({ symbol = '£', currency = 'GBP', pricesData = {} }) {
    ...
    const Toggle = ({ on, onClick }) => (
        <button onClick={onClick} className={`w-11 h-6 rounded-full transition-all relative shrink-0 ${on ? 'bg-brand3' : 'bg-gray-200'}`}>
            <div className={`absolute top-1 bg-white w-4 h-4 rounded-full transition-transform shadow-sm ${on ? 'left-6' : 'left-1'}`} />
        </button>
    );
        ...
        <div className="max-w-7xl mx-auto px-4 py-8 relative animate-in fade-in">
            ...
            <header className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex gap-3">
                    <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200 flex flex-col min-w-[170px]">
            ...
            {/* Summary bar + add */}
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex flex-col md:flex-row items-center justify-between gap-6 mb-6">
