      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
        {selections.map((sel, idx) => {
          ...
          return (
            <div
              key={sel.id}
              className="p-3.5 rounded-2xl border border-slate-100 bg-white shadow-sm relative flex flex-col gap-2"
            >
              {/* Card header */}
              ...
              <div className="w-full flex justify-between items-center">
                ...
                {customStats && (
                  <button
                    ...
                    className="text-[11px] font-semibold text-slate-600 hover:text-slate-900 bg-white border shadow-sm px-2 py-0.5 rounded-md transition-colors"
                  >
                    ✕ Clear Delta
                  </button>
                )}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {structuralMetrics.map((metrics, idx) => {
                  ...
                  return (
                    <div
                      key={metrics.id}
                      className="rounded-xl border border-slate-100 bg-white overflow-hidden transition-all shadow-sm"
                    >
                      {/* Summary header */}
                        ...
                        <div className="p-3 bg-slate-50/40 animate-in slide-in-from-top-1 duration-150">
                          ...
                          <div className="space-y-0.5 max-h-[220px] overflow-y-auto">
                            {metrics.holdings.map((fund, fIdx) => (
                              <div
                                key={fIdx}
                                className="grid grid-cols-[80px_1fr_52px_52px_64px] gap-x-2 items-center px-2 py-1.5 bg-white rounded-lg border border-slate-100 text-xs hover:bg-slate-50 transition-colors"
                              >
                                {/* Ticker */}
                                <div className="flex items-center gap-1.5 min-w-0">
